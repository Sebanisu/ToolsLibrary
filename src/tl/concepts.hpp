...
template<typename T>
concept is_resizable = requires(T t)
{
  t.resize(1U);
};
...
template<typename T>
concept is_continuous_range = is_range<T> && requires(T t)
{
...
concept is_trivial_or_continuous_range =
  is_nontrivial_continuous_range<T> || is_trivially_copyable<T>;
template<typename... outvarT>
concept size_greater_than_1 = sizeof...(outvarT) > 1U;
template<typename... outvarT>
concept is_not_empty = sizeof...(outvarT) != 0U;
template<typename... outvarT>
concept is_empty = !is_not_empty<outvarT...>;
}// namespace tl::concepts
