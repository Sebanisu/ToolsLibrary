...
requires(std::is_same_v<outputT, inputT>) void append(outputR &     out,
                                                      const inputR &in)
{
...
template<concepts::is_range inputR,
         typename inputT = std::decay_t<typename inputR::value_type>>
requires(std::is_same_v<inputT, char>) void append(std::ostream &out,
                                                   const inputR &in)
{
...
requires(((!std::is_same_v<inputT, char>)&&(
  concepts::is_trivially_copyable<inputT>))) void append(std::ostream &out,
                                                         const inputR &in)
{
  auto tmp = utility::to_bytes<inputT>(std::data(in), std::size(in));
  append(out, tmp);
...
template<concepts::is_contiguous_with_insert outputR,
         ...
         typename... Ts>
requires(std::is_same_v<std::decay_t<Ts>, T> &&...) void append(outputR &out,
                                                                const Ts &...in)
{
...
requires(
  ...
  && std::is_same_v<
    T,
    char> && (concepts::is_trivially_copyable<Ts> && ...)) void append(outputR
                                                                         &out,
                                                                       const Ts
                                                                         &...in)
{
...
requires((!std::is_same_v<Ts, char> || ...)
         && (concepts::is_trivially_copyable<
               Ts> && ...)) void append(std::ostream &out, const Ts &...in)
{
  ((append(out, utility::to_bytes(in))), ...);
}
// struct output
